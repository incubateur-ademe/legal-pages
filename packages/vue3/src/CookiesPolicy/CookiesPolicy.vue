...
withDefaults(defineProps<CookiesPolicyProps<object>>(), {
...
<template>
  <CookiesPolicyWithBeta v-if="includeBetaGouv" v-bind="{ ...$attrs, ...$props }" />
  <CookiesPolicy v-else v-bind="{ ...$attrs, ...$props }" />
</template>
