...
import { type PrivacyPolicyPropsThirdParty } from "@incubateur-ademe/legal-pages-markdown";

defineProps<{ thirdParties: PrivacyPolicyPropsThirdParty[] }>();
</script>
...
<template>
  <div className="fr-table fr-table--no-caption">
    <table>
      ...
      <tbody>
        <tr v-for="(thirdParty, index) in thirdParties" :key="index">
          <td>{{ thirdParty.name }}</td>
          <td>{{ thirdParty.country }}</td>
          <td>{{ thirdParty.hostingCountry }}</td>
          <td>{{ thirdParty.serviceType }}</td>
          <td>
            <a :href="thirdParty.policyUrl" rel="noopener noreferrer" target="_blank">{{ thirdParty.policyUrl }}</a>
          </td>
        </tr>
      </tbody>
    </table>
